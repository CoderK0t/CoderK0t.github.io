...
async def process_payment_request(message: Message, data: Dict[str, Any], user: types.User):
    ...
    # Для Telegram Stars важно использовать правильную структуру
    prices = [LabeledPrice(label="Игровая валюта", amount=amount * 100)]  # Умножаем на 100 для копеек/центов
    ...
    try:
        ...
        await message.answer_invoice(
            ...
            prices=prices,
            max_tip_amount=0,  # Для Stars обычно 0
            suggested_tip_amounts=[],  # Пустой массив для Stars
...
async def balance_command(message: Message):
    ...
    # Исправлено: убраны некорректные HTML-теги
    balance_text = (
        f"💰 <b>Ваш баланс:</b> {balance} единиц\n\n"
        f"Для пополнения используйте Mini App или команду /buy количество"
    )
...
async def buy_command(message: Message, command: CommandObject):
    try:
        ...
        if amount < 10:
            await message.answer("❌ Минимальная сумма пополнения: 10 единиц")
            return
